...
export async function createEmployee(
  ...
  // Rémunération initiale (facultative) : si un salaire de base ou des lignes de
  // primes sont renseignés, on crée directement la première entrée de l'historique.
  const initialSalary = Number(formData.get("initial_base_salary") ?? 0);
  const bonusLabels = formData.getAll("initial_bonus_label") as string[];
  const bonusAmounts = formData.getAll("initial_bonus_amount") as string[];
  const bonusItems = bonusLabels
    .map((label, i) => ({ label: label.trim(), amount: Number(bonusAmounts[i] ?? 0) }))
    .filter((l) => l.label.length > 0);

  if (initialSalary > 0 || bonusItems.length) {
