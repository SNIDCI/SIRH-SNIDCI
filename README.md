...
3. Va dans **SQL Editor** et exécute, **dans l'ordre** :
   ...
   - le contenu de `supabase/migrations/0005_compensation_bonus_items.sql` (primes
     structurées dans l'historique de rémunération)
...
## Nouveau : corrections PDF, primes structurées, suppression de période, invitations

- **PDF de paie** : remplacement de la librairie de génération (`pdf-lib` au lieu
  de `@react-pdf/renderer`, qui plantait sur Vercel). Plus de nouvelle dépendance
  cachée à gérer.
- **Primes/avantages structurés** : à la création d'un employé et dans l'onglet
  Rémunération, tu peux désormais ajouter autant de lignes "intitulé + montant"
  que nécessaire (bouton "+ Ajouter un intitulé"). Toutes ces lignes (salaire de
  base compris) sont reprises automatiquement comme suggestion lors de la
  création du bulletin de paie correspondant.
- **Suppression d'une période de paie** : possible depuis la liste des périodes
  (menu Paie), avec confirmation. Supprime aussi les bulletins PDF associés.
- **Invitations de comptes** : la validation du lien nécessite maintenant un
  vrai clic (page "Bienvenue, continuer") plutôt qu'une vérification automatique
  au chargement — certains filtres de sécurité d'e-mail professionnels
  (Outlook, Gmail Workspace) ouvrent les liens automatiquement pour les
  scanner, ce qui invalidait le lien avant que la personne ne clique elle-même.
