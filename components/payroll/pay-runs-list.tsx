...
import { createPayRun, deletePayRun } from "@/lib/actions/payroll";
  ...
  return (
    <div>
      ...
      <div className="mt-6 panel divide-y divide-line">
        {payRuns.map((p) => (
          <div key={p.id} className="flex items-center justify-between px-4 py-3 hover:bg-canvas/40">
            <Link href={`/payroll/${p.id}`} className="flex flex-1 items-center gap-3">
            ...
            <DeleteButton
              action={deletePayRun.bind(null, p.id)}
              confirmMessage={`Supprimer définitivement la période "${p.period_label}" et tous ses bulletins ? Cette action est irréversible.`}
            />
          </div>
        ))}
        {!payRuns.length && (
