  ...
  return (
    <div className="space-y-6">
      <p className="text-xs text-slate">
        ...
        l&apos;émission des bulletins de paie se font dans le module Paie, qui reprend automatiquement
        la dernière entrée saisie ici comme suggestion.
      </p>
      ...
      <form action={formAction} className="panel space-y-4 p-5">
        ...
        <BonusLinesInput fieldPrefix="bonus" />
            ...
            <div key={e.id} className="px-4 py-3 text-sm">
              <div className="flex items-center justify-between">
                <span className="font-medium text-ink">
                  {e.base_salary.toLocaleString("fr-FR")} F{" "}
              ...
              {e.bonus_items?.length > 0 && (
                <ul className="mt-1 space-y-0.5">
                  {e.bonus_items.map((b, i) => (
                    <li key={i} className="flex justify-between text-slate">
                      <span>{b.label}</span>
                      <span>{b.amount.toLocaleString("fr-FR")} F</span>
                    </li>
                  ))}
                </ul>
              )}
