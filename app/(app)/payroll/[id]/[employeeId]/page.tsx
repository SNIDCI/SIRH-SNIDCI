...
export default async function PayslipEditPage({
  ...
  const [{ data: payRun }, { data: employee }, { data: payslip }, { data: latestComp }] = await Promise.all([
    ...
    supabase
      ...
      .select("base_salary, bonus_items")
  ...
  const suggestedLines = latestComp
    ? [
        { label: "Salaire de base", amount: latestComp.base_salary },
        ...((latestComp.bonus_items as { label: string; amount: number }[]) ?? []),
      ]
    : [];
